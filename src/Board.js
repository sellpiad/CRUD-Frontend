        ...
        <div>
            <StyledTable>
                ...
                <tbody>
                    {
                        headers.length > 0 ?
                            Object.values(headers).map((row) => (
                                <StyledRow>
                                    {
                                        Object.keys(row).map((data) => (
                                            data === 'title' ?
                                                <StyledCell><div onClick={() => postModalHandler(row['id'])}>{row[data]}</div></StyledCell>
            ...
            <button type="button" onClick={createButtonHandler}>글 등록</button>
